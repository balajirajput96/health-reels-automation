...
meta = load('assets/reel_0076_metadata.json')
qc = load('assets/reel_0076_qc_report.json')
...
assert abs(checks['duration_seconds'] - 72.767) < 0.01
...
assert abs(meta['measured_narration_duration_seconds'] - 72.76) < 0.01
assert meta['qc']['local_media_qc'] == 'passed'
assert meta['qc']['drive_verified'] is True and meta['qc']['drive_manifest_uploaded'] is True
assert meta['qc']['drive_manifest_file_id'] == (ROOT / 'state/reel_0076_manifest_file_id.txt').read_text().strip()
...
attempts = (ROOT / 'state/reel_0076_audio_attempts.txt').read_text(encoding='utf-8')
assert attempts.count('attempt1\t72.76\taccepted_canonical') == 1
assert 'DURATION 72.76' in (ROOT / 'state/reel_0076_audio_probe_attempt1.txt').read_text(encoding='utf-8')
sha = (ROOT / 'assets/reel_0076_procrastination_what_studies_measure_hi.mp4').read_bytes()
mp4_sha = hashlib.sha256(sha).hexdigest()
assert mp4_sha == meta['measured_mp4_sha256']
drive = load('state/reel_0076_drive_final_verification.json')
...
row = next(item for item in queue if item['sequence'] == 76)
...
assert row['qc']['duration_seconds'] == 72.767
assert row['asset_checksums']['reel_0076_procrastination_what_studies_measure_hi.mp4'] == mp4_sha
ledger = load('state/reels_ledger.json')['items']
records = [item for item in ledger if item.get('source_id') == 'reel_0076_procrastination_what_studies_measure' and item.get('stage') == 'final']
assert len(records) == 1 and records[0]['sha256'] == mp4_sha
checkpoint = load('state/reels_3000_checkpoint.json')
assert checkpoint['production_counts'] == {'final': 76, 'planned': 2924}
assert checkpoint['completed_drive_verified'] == 76
assert checkpoint['next_sequence'] == 77
assert checkpoint['next_reel'] == 'reel_0077_stress_appraisal_what_studies_measure'
manifest = load('state/reel_0076_drive_upload_manifest.json')
assert len(manifest['files']) == 11 and manifest['object_count'] == 11
assert manifest['drive_folder_id'] == (ROOT / 'state/reel_0076_drive_folder_id.txt').read_text().strip()
assert any(item['name'] == 'reel_0076_procrastination_what_studies_measure_hi.mp4' and item['local_sha256'] == mp4_sha for item in manifest['files'])
print('R0076_FINAL_VALIDATION_OK')
