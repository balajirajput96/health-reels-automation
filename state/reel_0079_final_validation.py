...
EXPECTED_SOURCE_IDS = {
    'doi:10.3389/fpsyg.2022.943065',
    'doi:10.3390/healthcare11182515',
    'doi:10.1017/S0954579425000197',
    'doi:10.1002/smi.3213',
}
meta = load('assets/reel_0079_metadata.json')
qc = load('assets/reel_0079_qc_report.json')
...
assert abs(checks['duration_seconds'] - 68.167) < 0.01
...
assert abs(meta['measured_narration_duration_seconds'] - 68.20) < 0.01
assert meta['qc']['local_media_qc'] == 'passed'
assert meta['qc']['drive_verified'] is True and meta['qc']['drive_manifest_uploaded'] is True
assert meta['qc']['drive_manifest_file_id'] == (ROOT / 'state/reel_0079_manifest_file_id.txt').read_text().strip()
assert meta['qc']['drive_folder_id'] == (ROOT / 'state/reel_0079_drive_folder_id.txt').read_text().strip()
...
attempts = (ROOT / 'state/reel_0079_audio_attempts.txt').read_text(encoding='utf-8')
assert attempts.count('attempt1\t79.36\tfailed_duration') == 1
assert attempts.count('attempt2\t68.20\taccepted_canonical') == 1
assert 'duration=79.360000' in (ROOT / 'state/reel_0079_audio_probe_attempt1.txt').read_text(encoding='utf-8')
assert 'duration=68.200000' in (ROOT / 'state/reel_0079_audio_probe_attempt2.txt').read_text(encoding='utf-8')
...
drive = load('state/reel_0079_drive_final_verification.json')
...
row = next(item for item in queue if item['sequence'] == 79)
...
assert abs(row['qc']['duration_seconds'] - 68.167) < 0.01
assert row['asset_checksums']['reel_0079_recovery_what_studies_measure_hi.mp4'] == mp4_sha
assert set(row['source_ids']) == EXPECTED_SOURCE_IDS
ledger = load('state/reels_ledger.json')['items']
records = [item for item in ledger if item.get('source_id') == 'reel_0079_recovery_what_studies_measure' and item.get('stage') == 'final']
assert len(records) == 1 and records[0]['sha256'] == mp4_sha
checkpoint = load('state/reels_3000_checkpoint.json')
assert checkpoint['production_counts'] == {'final': 79, 'planned': 2921}
assert checkpoint['completed_drive_verified'] == 79
assert checkpoint['next_sequence'] == 80
assert checkpoint['next_reel'] == 'reel_0080_social_buffering_what_studies_measure'
manifest = load('state/reel_0079_drive_upload_manifest.json')
assert len(manifest['files']) == 11 and manifest['object_count'] == 11
assert manifest['drive_folder_id'] == (ROOT / 'state/reel_0079_drive_folder_id.txt').read_text().strip()
remote = {item['name']: item for item in load('state/reel_0079_drive_listing_final.json')['files']}
...
print('R0079_FINAL_VALIDATION_OK')
