...
source_id = 'reel_0080_social_buffering_what_studies_measure'
...
if matching:
    if len(matching) == 1 and matching[0].get('stage') == 'final' and matching[0].get('sha256') == sha:
        print('R0080_LEDGER_ALREADY_REGISTERED')
        print(json.dumps(matching[0], ensure_ascii=False, indent=2))
        raise SystemExit(0)
    raise SystemExit(f'conflicting R0080 ledger records: {matching!r}')
record = {
    'source_id': source_id,
    'sha256': sha,
    'filename': 'reel_0080_social_buffering_what_studies_measure_hi.mp4',
    'stage': 'final',
    'target_account': '@balajirajput96',
    'recorded_at': datetime.now(UTC).isoformat(timespec='microseconds').replace('+00:00', 'Z'),
    'notes': 'R0080 final: 4 fresh verified sources; local QC passed; 71.167s encoded 1080x1920 H.264/AAC reel; canonical Drive exact package independently verified; not published.',
}
items.append(record)
