...
FOLDER_ID = (ROOT / 'state/reel_0080_drive_folder_id.txt').read_text(encoding='utf-8').strip()
MANIFEST_ID = (ROOT / 'state/reel_0080_manifest_file_id.txt').read_text(encoding='utf-8').strip()
meta = load('assets/reel_0080_metadata.json')
qc = load('assets/reel_0080_qc_report.json')
...
assert abs(checks['duration_seconds'] - 71.167) < 0.01
...
assert abs(meta['measured_narration_duration_seconds'] - 71.16) < 0.01
...
attempts = (ROOT / 'state/reel_0080_audio_attempts.txt').read_text(encoding='utf-8')
assert 'attempt1: duration=90.600s' in attempts and 'status=FAILED_DURATION_OUT_OF_RANGE' in attempts
assert 'attempt2: duration=71.160s' in attempts and 'status=ACCEPTED_CANONICAL' in attempts
assert '"duration": "90.600000"' in (ROOT / 'state/reel_0080_audio_probe_attempt1.txt').read_text(encoding='utf-8')
assert '"duration": "71.160000"' in (ROOT / 'state/reel_0080_audio_probe_attempt2.txt').read_text(encoding='utf-8')
assert '"duration": "71.160000"' in (ROOT / 'state/reel_0080_canonical_audio_probe.txt').read_text(encoding='utf-8')
...
drive = load('state/reel_0080_drive_final_verification.json')
...
row = next(item for item in queue if item['sequence'] == 80)
...
assert abs(row['qc']['duration_seconds'] - 71.167) < 0.01
assert row['asset_checksums']['reel_0080_social_buffering_what_studies_measure_hi.mp4'] == mp4_sha
assert set(row['source_ids']) == EXPECTED_SOURCE_IDS
ledger = load('state/reels_ledger.json')['items']
records = [item for item in ledger if item.get('source_id') == 'reel_0080_social_buffering_what_studies_measure' and item.get('stage') == 'final']
assert len(records) == 1 and records[0]['sha256'] == mp4_sha
checkpoint = load('state/reels_3000_checkpoint.json')
assert checkpoint['production_counts'] == {'final': 80, 'planned': 2920}
assert checkpoint['completed_drive_verified'] == 80
assert checkpoint['next_sequence'] == 81
assert checkpoint['next_reel'] == 'reel_0081_stress_and_attention_what_studies_measure'
manifest = load('state/reel_0080_drive_upload_manifest.json')
assert len(manifest['files']) == 11 and manifest['object_count'] == 11
assert manifest['drive_folder_id'] == FOLDER_ID
remote = {item['name']: item for item in load('state/reel_0080_drive_listing_final.json')['files']}
...
print('R0080_FINAL_VALIDATION_OK')
