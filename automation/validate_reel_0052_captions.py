...
p = Path('assets/reel_0052_choice_overload_what_studies_measure_captions_hi.srt')
...
for b in blocks:
    ...
    assert start >= prev and end > start and end <= 65.92, (start, end, prev)
    prev = end

safety = ' '.join(' '.join(b[2:]) for b in blocks[-2:]).lower()
assert 'diagnosis' in safety and 'advice' in safety
