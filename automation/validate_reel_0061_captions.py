...
import re
p=Path('assets/reel_0061_memory_reconsolidation_what_studies_measure_captions_hi.srt')
...
for i,b in enumerate(blocks,1):
 ...
 assert start>=prev and end>start and end<=71.52,(start,end,prev)
 prev=end
safety=' '.join(' '.join(b[2:]) for b in blocks).lower()
for term in ('diagnosis','advice','direct read','guaranteed therapy'): assert term in safety,term
