...
import re
p=Path('assets/reel_0066_deep_work_claims_what_studies_measure_captions_hi.srt')
...
for i,b in enumerate(blocks,1):
    ...
    assert start>=prev and end>start and end<=72.92,(start,end,prev)
    prev=end
text=' '.join(' '.join(b[2:]) for b in blocks).lower()
for term in ('diagnosis','advice','deep work','universal','ai narration'):
