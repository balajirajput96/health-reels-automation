...
import re
p=Path('assets/reel_0068_social_learning_what_studies_measure_captions_hi.srt')
...
for i,b in enumerate(blocks,1):
    ...
    assert start>=prev and end>start and end<=62.76,(start,end,prev)
    prev=end
text=' '.join(' '.join(b[2:]) for b in blocks).lower()
for term in ('सीधे mind-reading नहीं','diagnosis','personal assessment','ai-generated educational visuals'):
