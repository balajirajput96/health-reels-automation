...
EXPECTED={'reel_0061_memory_reconsolidation_what_studies_measure_hi.mp4','reel_0061_memory_reconsolidation_what_studies_measure_narration_hi.wav','reel_0061_memory_reconsolidation_what_studies_measure_captions_hi.srt','reel_0061_metadata.json','reel_0061_qc_report.json','reel_0061_memory_reconsolidation_what_studies_measure_sources.md','reel_0061_memory_reconsolidation_what_studies_measure_script.md','reel_0061_scene_01.png','reel_0061_scene_02.png','reel_0061_scene_03.png','reel_0061_scene_04.png'}
def main():
 ...
 r={'folder_id':a.folder_id,'manifest_id':a.manifest_id,'expected_count':len(EXPECTED),'found_expected_count':len(current),'missing':missing,'zero_size':zero,'wrong_parent':wrong,'manifest_found_once':len(manifests)==1,'manifest_parent_ok':len(manifests)==1 and a.folder_id in (manifests[0].get('parents') or []),'manifest_nonzero':len(manifests)==1 and int(manifests[0].get('size',0) or 0)>0,'unrelated_remote_object_count':max(0,len(files)-len(current)-len(manifests)),'unrelated_remote_objects':sorted(f.get('name') for f in files if f.get('name') not in EXPECTED and f.get('id')!=a.manifest_id),'expected_objects':[{'name':n,'id':current[n].get('id'),'size':int(current[n].get('size',0) or 0),'mimeType':current[n].get('mimeType')} for n in sorted(current)]}
 r['valid']=not(missing or zero or wrong or len(manifests)!=1 or not r['manifest_parent_ok'] or not r['manifest_nonzero'] or r['unrelated_remote_object_count']!=0); text=json.dumps(r,ensure_ascii=False,indent=2)+'\n'
