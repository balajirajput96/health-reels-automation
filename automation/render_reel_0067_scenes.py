...
def arrow(d,a,b,c):
    d.line((a[0],a[1],b[0],b[1]),fill=(*c,210),width=11)
    import math
    ang=math.atan2(b[1]-a[1],b[0]-a[0]); size=30
    pts=[b,(b[0]-size*math.cos(ang-.5),b[1]-size*math.sin(ang-.5)),(b[0]-size*math.cos(ang+.5),b[1]-size*math.sin(ang+.5))]
    d.polygon(pts,fill=(*c,220))

def scene1():
    im=base(); d=ImageDraw.Draw(im,'RGBA'); glow(im,720,400,590,CYAN); card(d,(85,690,1355,1515),CYAN)
    for x,y,c in [(260,1030,CYAN),(520,890,TEAL),(860,1110,AMBER),(1180,920,VIOLET)]:
        node(d,x,y,c,43); d.line((x,y+55,x,y+170),fill=(*WHITE,130),width=8)
    d.rounded_rectangle((380,1160,1060,1340),radius=32,fill=(7,18,43,230),outline=(*WHITE,130),width=5)
    d.line((500,1250,940,1250),fill=(*CYAN,210),width=12)
    for x in (500,610,720,830,940): node(d,x,1250,CYAN,16)
    footer(d); return im

def scene2():
    im=base(); d=ImageDraw.Draw(im,'RGBA'); glow(im,720,390,560,AMBER); card(d,(80,670,1360,1570),AMBER)
    d.rounded_rectangle((130,790,650,1430),radius=36,fill=(9,26,54,225),outline=(*TEAL,210),width=6)
    d.rounded_rectangle((790,790,1310,1430),radius=36,fill=(9,26,54,225),outline=(*ROSE,210),width=6)
    for x,y,c in [(300,980,TEAL),(480,1250,TEAL),(950,980,ROSE),(1130,1250,ROSE)]: node(d,x,y,c,36)
    arrow(d,(335,980),(445,1250),TEAL); arrow(d,(985,980),(1095,1250),ROSE)
    d.line((650,1110,790,1110),fill=(*WHITE,180),width=7)
    node(d,720,1110,WHITE,30)
    footer(d); return im
...
def scene3():
    ...
    for x1,y1,x2,y2,c in boxes:
        d.rounded_rectangle((x1,y1,x2,y2),radius=34,fill=(8,23,50,240),outline=(*c,220),width=6)
        d.line((x1+65,y2-65,x2-65,y2-65),fill=(*WHITE,130),width=7)
...
def scene4():
    ...
    for x,y,c in centers:
        d.ellipse((x-105,y-105,x+105,y+105),fill=(9,24,52,235),outline=(*c,220),width=7); node(d,x,y,c,32)
    for a,b in zip(centers,centers[1:]): arrow(d,(a[0]+110,a[1]),(b[0]-110,b[1]),WHITE)
    d.line((230,1300,1210,1300),fill=(*WHITE,175),width=8)
    for x,h,c in [(270,160,CYAN),(500,260,TEAL),(730,210,AMBER),(960,320,VIOLET),(1190,180,ROSE)]:
        d.rounded_rectangle((x,1300-h,x+90,1300),radius=18,fill=(*c,210),outline=(*WHITE,120),width=4)
    footer(d); return im

for i,fn in enumerate((scene1,scene2,scene3,scene4),1):
    path=OUT/f'reel_0067_scene_{i:02d}.png'; fn().save(path,optimize=True); print(path)
