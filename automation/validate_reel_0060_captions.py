...
import re
p=Path('assets/reel_0060_false_memory_what_studies_measure_captions_hi.srt')
...
assert len(blocks)==21,len(blocks); prev=0.0
for b in blocks:
 ...
 assert start>=prev and end>start and end<=71.92,(start,end,prev); prev=end
safety=' '.join(' '.join(b[2:]) for b in blocks).lower()
assert 'diagnosis' in safety and 'advice' in safety and 'honesty' in safety
