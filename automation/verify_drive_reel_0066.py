...
EXPECTED={'reel_0066_deep_work_claims_what_studies_measure_hi.mp4','reel_0066_deep_work_claims_what_studies_measure_narration_hi.wav','reel_0066_deep_work_claims_what_studies_measure_captions_hi.srt','reel_0066_metadata.json','reel_0066_qc_report.json','reel_0066_deep_work_claims_what_studies_measure_sources.md','reel_0066_deep_work_claims_what_studies_measure_script.md','reel_0066_scene_01.png','reel_0066_scene_02.png','reel_0066_scene_03.png','reel_0066_scene_04.png'}
def main():
 ...
 missing=sorted(EXPECTED-set(current)); zero=sorted(n for n,f in current.items() if int(f.get('size',0) or 0)<=0); wrong=sorted(n for n,f in current.items() if a.folder_id not in (f.get('parents') or [])); duplicate_names=sorted(n for n in EXPECTED if sum(1 for f in files if f.get('name')==n)>1)
