...
import re
p=Path('assets/reel_0064_mind_wandering_what_studies_measure_captions_hi.srt')
...
for i,b in enumerate(blocks,1):
    ...
    assert start>=prev and end>start and end<=70.32,(start,end,prev); prev=end
text=' '.join(' '.join(b[2:]) for b in blocks).lower()
for term in ('diagnosis','advice','direct thought-reading','live recording'): assert term in text,term
