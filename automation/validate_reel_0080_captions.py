...
SRT = ROOT / 'assets/reel_0080_social_buffering_what_studies_measure_captions_hi.srt'
text = SRT.read_text(encoding='utf-8')
blocks = [b for b in re.split(r'\n\s*\n', text.strip()) if b.strip()]
assert len(blocks) == 15, len(blocks)
last_end = 0
required = ['सामाजिक बफ़रिंग', 'perceived support', 'received support', 'cortisol', 'experience sampling', 'causation', 'biomarker', 'AI-सहायित', 'व्यक्तिगत सलाह नहीं']
full = ''
for index, block in enumerate(blocks, 1):
    ...
    last_end = end
    full += '\n'.join(lines[2:]) + '\n'
assert last_end == 71_160, last_end
for phrase in required:
    assert phrase in full, phrase
print('R0080_CAPTIONS_OK')
